# Fundamentos de trabajar con IA

Versión organizada del curso, dividida por bloques para facilitar su enseñanza, edición y actualización.

## Contenido principal

1. [Introducción, escalera e índice](./curso/00-introduccion.md)
2. [Nivel 1 — Entender y conversar](./curso/01-nivel-1-entender-y-conversar.md)
3. [Nivel 2 — Conectar y potenciar](./curso/02-nivel-2-conectar-y-potenciar.md)
4. [Nivel 3 — Delegar trabajo](./curso/03-nivel-3-delegar-trabajo.md)
5. [Nivel 4 — Construir sistemas](./curso/04-nivel-4-construir-sistemas.md)
6. [Proyectos, tarea final, glosario y mapa de decisión](./curso/05-proyectos-tarea-glosario-mapa.md)

## Material complementario

- [Banco de escenarios por audiencia](./banco-de-escenarios.md)

## Lugar en la ruta

Este curso corresponde al número **02** y se recomienda después de:

- [**01 · Cómo comunicarnos y trabajar con la IA**](../01-comunicarnos-trabajar-con-ia/README.md)

El curso 01 enseña a diseñar instrucciones, contexto y ciclos de trabajo. Este curso 02 explica las piezas tecnológicas que hacen posibles sistemas más avanzados: LLM, tokens, modelos, RAG, MCP, Skills, agentes, Claude Code, hooks y automatización.

## Evitar repetición

Las secciones iniciales sobre conversación y prompting deben utilizarse como repaso. El instructor debe concentrar la profundidad en:

- funcionamiento y límites de los modelos;
- recuperación de información;
- conexiones y herramientas;
- delegación;
- selección del nivel correcto de autonomía.